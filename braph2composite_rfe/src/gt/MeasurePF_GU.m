classdef MeasurePF_GU < MeasurePF
	...
	properties (Constant) % properties
		LAYER = MeasurePF.getPropNumber() + 1;
		LAYER_TAG = 'LAYER';
	...
	methods (Static) % inspection
		...
		function tag = getPropTag(pointer)
			...
			if ischar(pointer)
				...
				switch prop
					case MeasurePF_GU.LAYER
						tag = MeasurePF_GU.LAYER_TAG;
					otherwise
		...
		function prop_category = getPropCategory(pointer)
			...
			switch prop
				case MeasurePF_GU.LAYER
					prop_category = MeasurePF_GU.LAYER_CATEGORY;
				otherwise
		...
		function prop_format = getPropFormat(pointer)
			...
			switch prop
				case MeasurePF_GU.LAYER
					prop_format = MeasurePF_GU.LAYER_FORMAT;
				otherwise
		...
		function prop_description = getPropDescription(pointer)
			...
			switch prop
				...
				case MeasurePF_GU.ELCLASS
					prop_description = 'ELCLASS (constant, string) is the class of the panel figure for global unilayer measure (MeasurePF_GU).';
		...
		function prop_check = checkProp(pointer, value)
			...
			switch prop
				case MeasurePF_GU.LAYER % __MeasurePF_GU.LAYER__
					check = Format.checkFormat(Format.SCALAR, value, MeasurePF_GU.getPropSettings(prop));
